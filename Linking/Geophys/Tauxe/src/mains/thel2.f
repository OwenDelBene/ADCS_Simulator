...
c	open(6,carriagecontrol="list")
 ...
 10      write(*,*)'Usage: thellier [-sfm][min max field][Standard I/O]'
     ...
     1   f, g, s/|b|, MAD, alpha, drat,accept (y/n)'
...
c
	call fpint(s,ndat,nmag,magdat,plt,ptrm,nplt,nptrm,pline,dat)
c
...
c
	write(*,'(a)')'dash 0 0'
	write(*,'(a)')'read 2'
	write(*,*)bline(1,2),bline(1,1)
	write(*,*)bline(2,2),bline(2,1)
	write(*,'(a)')'symb 1 .2'
c
c	plot ptrm-checks
c
...
c
	call plotdmag(s,nmag,dat,is,ie,theta,mad,ipow)
...
9992	format(a27,1x,a3)
	...
	do 777 i=1,10000
 777	read(10,*,end=788)
 9990	format(a15,4(i4,1x),i6,1x,f5.3,1x,f6.1,1x,3(f5.3,1x),3(f6.1,1x),a3)
 788	write(10,9990)
     1   s,min,max,ngood,ilab,ifield,slop,qbest,f,g,
     1   scat,MAD,theta,drat,accept
	close(unit=10)
	end
c______________________________________________________
	subroutine fpint(s,n,nummag,dat,plt,ptrm,nplt,nptrm,pline,tmp)
c
...
20      IF(J.LE.IR)THEN
      ...
      END
c******************************************************************
	subroutine plotdmag(s,nn,dat,istart,iend,angle,mad,ipow)
