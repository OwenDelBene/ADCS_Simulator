...
C in order to place these routines in dll, they should
C be compiled with large model and /Aw option
C
!DEC$ INTEGER:4
...
30    ICODE=3                                                           
      T=-P(3,N-1)/STEP                                                  
      FL=1.0/(ABS(((C3*T+C2)*T+C1)*T+C0)+1E-15)                          
      RETURN                                                            
      END                                                               
C
C
...
C-----------------------------------------------------------------------
      ...
      CHARACTER*12 	NAME
      COMMON 		XI(3),H(216)
      COMMON/MODEL/	NAME,NMAX,TIME,G(216)  
      COMMON/GENER/	UMR,ERA,AQUAD,BQUAD
C
...
C*****ENTRY POINT  FELDC  TO BE USED WITH CARTESIAN CO-ORDINATES        
      ...
      YYY=V(2)                                                          
      ZZZ=V(3)                                                          
10    RQ=1.0/(XXX*XXX+YYY*YYY+ZZZ*ZZZ) 
      XI(1)=XXX*RQ                                                      
      XI(2)=YYY*RQ                                                      
      XI(3)=ZZZ*RQ                                                      
      GOTO20                                                            
...
C     if DIMO <0 then an error detected in reading coeffcients
C         in which case DIMO contains fortran run time error
C  D. BILITZA, NSSDC, GSFC, CODE 633, GREENBELT, MD 20771, 
C	(301)286-9536   NOV 1987.
C-----------------------------------------------------------------------
C  to update with new igrf
C     add igrf .dat file to data/filmod, add years to data/dtemod
C   add 1 to numye= value
! dimensions of GH1, GH2 and GHA should be max coeff *13 +max coeff. i.e 13*13 +13
	CHARACTER*12	FILMOD, FIL1, FIL2           
	DIMENSION	GH1(216),GH2(216),GHA(216)
...
C
C ### numye = numye + 1 ; is number of years represented by IGRF one less than files in FILMOD 
C
	NUMYE=NFILES-1
	if(YEAR <YSTART) then
		DIMO=-5;
...
1234	CONTINUE
	...
	I = I+1                                         
      DO 9 M=1,N                                    
      	F = F * (X + M) / (X - M + 1.D0)                 
	IF(IS.EQ.0) F = F * DSQRT((X - float(M) + 1.D0) / (X + float(M)))      	
	GH1(I) = GHA(I-1) * F
	GH1(I+1) = GHA(I) * F
        I=I+2
9     CONTINUE                                          
